"""Pre process video reviews data."""
...
def preprocess(
    s3_in_url: str, s3_out_bucket: str, s3_out_prefix: str, delimiter: str
) -> str:
    """
    ...
    :param s3_in_url: s3 url to the delimited file to be processed
        e.g. s3://amazon-reviews-pds/tsv/reviews.tsv.gz
    :param s3_out_bucket: s3 bucket where preprocessed data will be staged
    :param s3_out_prefix: s3 url prefix to stage preprocessed data to use later in the pipeline
        e.g. video-reviews/preprocess/
    :param delimiter: delimiter to be used for parsing the file. Defaults to "," if none provided
    :return: status of preprocessed data
    """
    try:
        ...
        dataframe = pd.read_csv(s3_in_url, sep=delimiter, on_bad_lines="warn")
        ...
        customers = dataframe["customer_id"].value_counts()
        products = dataframe["product_id"].value_counts()
        ...
        print("# of rows before the long tail = {:10d}".format(dataframe.shape[0]))
        reduced_df = dataframe.merge(
        ...
        with file_system.open(s3_out_train, "w") as file:
            train_df.to_csv(file, sep=str(","), index=False)
        ...
        with file_system.open(s3_out_validate, "w") as file:
            validate_df.to_csv(file, sep=str(","), index=False)

        s3_out_test = "s3://{}/{}/{}".format(
            s3_out_bucket, s3_out_prefix, "test/test.csv"
        )
        print("writing test data to {}".format(s3_out_test))
        with file_system.open(s3_out_test, "w") as file:
            test_df.to_csv(file, sep=str(","), index=False)
