...
class S3KeySizeSensor(S3KeySensor):
    """
    Waits for a key (a file-like instance on S3) to be present and be more than some size in a S3 bucket.
    ...
    def __init__(
        ...
        *,
        ...
        object_min_size: Union[int, float] = 0,
        **kwargs,
    ):
        super().__init__(**kwargs)
    ...
    def check_fn(self, data: List) -> bool:
        """
        Check that S3 Objects have size more than a given value.

        :param data: List of the objects in S3 bucket.
        :type data: list
        """
        return all(
            f.get("Size", 0) > self.object_min_size for f in data if isinstance(f, dict)
        )
